...
```php
FontsManager::withDriver(string $driver_name, Closure $closure): FontsManager
```

render the HTML code of every driver instantiated with at least 1 font set.
```php
FontsManager::render(): string
```

Print the code rendered by the method `render`
```php
